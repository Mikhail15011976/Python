...
select distinct al.name_artist, al2.year_album from artist_list al 
join artist_album aa on al.id_artist = aa.id_artist 
join album_list al2 on aa.id_album = al2.id_album 
where al.name_artist not in (select distinct al.name_artist from artist_list al 
join artist_album aa on al.id_artist = aa.id_artist 
join album_list al2 on aa.id_album = al2.id_album 
where al2.year_album = 2020)
order by al.name_artist;
...
join track_list tl on al.id_album = tl.id_album 
group by al.name_album 
having count(*) = (select count(*) from album_list al 
join track_list tl on al.id_album = tl.id_album 
group by al.name_album 
order by count(*)
limit 1);
